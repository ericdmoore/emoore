...
type ExpandLinkEvent = ExpandEventClick | ExpandEventAPI
...
export const getResponder: Responder<{found:ILink}> = async (d, e, c, extras) => {
  ...
  const geo = nullableGeo || undefined
  ...
  const ret : SRet = {
    statusCode: HTTPstatusCodes.TEMPORARY_REDIRECT,
    headers: {
      ...d.found.metatags,
      Location,
      'X-History': JSON.stringify(history)
    },
