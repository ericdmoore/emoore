...
// client/index.ts
var import_got = __toModule(require_source5());
var client = (baseURL) => {
  let authToken = "missing";
  const postTokens = (tokenResouce) => (input) => {
    if ("email" in input) {
      return import_got.default.post(`${baseURL}/${tokenResouce}`, {headers: {
  ...
  const tokens = {
    resource: "tokens",
    get: async (user, pass, TFA) => fetch(baseURL + tokens.resource, {method: "GET", headers: {authToken}}),
  ...
  const expand = {
    resource: "expand",
    get: (short) => import_got.default.get(`${baseURL}/${expand.resource}/${short}`, {headers: {authToken}})
  };
  const stats = {
    resource: "stats",
    get: () => import_got.default.get(`${baseURL}/${stats.resource}`, {headers: {authToken}})
  };
  const links = {
    resource: "links",
    get: () => import_got.default.get(`${baseURL}/${links.resource}`, {headers: {authToken}}),
    put: () => import_got.default.put(`${baseURL}/${links.resource}`, {headers: {authToken}}),
    post: () => import_got.default.post(`${baseURL}/${links.resource}`, {headers: {authToken}}),
    del: () => import_got.default.delete(`${baseURL}/${links.resource}`, {headers: {authToken}})
  };
  const users = {
    resource: "users",
    get: async () => import_got.default.get(`${baseURL}/${users.resource}`, {headers: {authToken}}),
    put: () => import_got.default.put(`${baseURL}/${users.resource}`, {headers: {authToken}}),
    post: () => import_got.default.post(`${baseURL}/${users.resource}`, {headers: {authToken}, json: {}}),
    del: () => import_got.default.delete(`${baseURL}/${users.resource}`, {headers: {authToken}})
  };
  const root = () => import_got.default.get(baseURL);
  return {
    get: {
      users: users.get,
      expand: expand.get,
