...
describe('Using the Local Dyanmo Service Test Harness',()=>{
    ...
    test('Add1 and Remove1 to l[0]',  async ()=>{
        const i = 0
        const {short, long}  = linksList[i]

        const now = Date.now()
        const cntBefore = await click.query.count(short, { stop: now + 10_000, start: now - 3600_000 })
        ...
        await click.batch.save([addMe])
        const cntAfterAdd = await click.query.count(short, { stop: now + 10_000, start: now - 3600_000, })
        await expect(cntAfterAdd.Count ?? -1).toBeGreaterThan(cntBefore.Count ?? 0)

        await click.batch.remove([addMe])
        const cntAfterRm = await click.query.count(short, { stop: now + 10_000, start: now - 3600_000, })
